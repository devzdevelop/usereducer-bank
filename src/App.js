...
function reducer(state, action) {
  console.log(state.isActive, action.type)
  //      false                     openAccount
  ...
  switch(action.type) {
    ...
    case 'requestLoan': {
      const newLoan = state.loan >= 0 && state.balance >= 0 ? action.payload : state.loan;
      
      return {
        ...state,
        loan:  newLoan,
        balance:  state.loan === 0 ? state.balance + newLoan : state.balance,
      }
    }
    case 'payLoan': {
      return {
        ...state,
        balance:  state.loan >= action.payload ? state.balance - action.payload : state.balance,
        loan:  0,
      }
    }
    case 'closeAccount': {
      const isEmpty =  !state.balance && !state.loan ? true : false;
      
      return {
        ...state,
        status:  isEmpty ? "false" : state.status,
        isActive: isEmpty ? true : state.isActive,
      }
    }
    default: {
